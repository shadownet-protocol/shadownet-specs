...
- RFC-0005: added §Provider DID discovery. SNS providers MUST expose `/.well-known/sns/v1/provider.json` at the apex returning the signing DID; resolvers MUST fetch it before verifying records, MUST treat the returned `providerDid` as authoritative, and MUST NOT derive the DID from the Shadowname's host. Closes a prior ambiguity where the spec example used a subdomain DID (`did:web:sns.sh4dow.org`) while implementations derived the DID from the apex — the two were not interoperable. §Resolution flow diagram and prose updated to include the discovery hop; `examples/birthday-flow.md` shows the new fetch.
